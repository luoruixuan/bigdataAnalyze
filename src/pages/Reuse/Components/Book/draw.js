var width = 850; // width of svg image
...
export function arcDiagram(divid, graph, setArc) {
...
function drawNodes(nodes) {
  ...
  d3.select('#plot')
    ...
    .on('click', function (d, i) {
      window.location.href = '/book/' + d.name;
    });
}

// Draws nice arcs for each link on plot
function drawLinks(links, setArc) {
  ...
  d3.select('#plot')
    ...
    .on('click', function (d, i) {
      setArc(d.source.name, d.target.name);
    });
}
