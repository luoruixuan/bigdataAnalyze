...
function warpTag(content: string, highlight: any, tagName: string) {
  ...
  if (highlight.length == 0) {
    ...
    var highlight_r = Math.min(
      highlight[highlight.length - 1][1],
      highlight_l + global.showlength,
    );
  }
  ...
  for (var i = 0; i < highlight.length; ++i) {
    var rg = highlight[i];
    var l = rg[0] - str_left,
      r = Math.min(rg[1] - str_left, trunced_str.length);
  ...
  if (now < trunced_str.length)
    lst.push(trunced_str.substr(now, trunced_str.length));
  var ret = lst.join('');
...
class ContentResult extends React.Component {
  ...
  getdata() {
    var data = [];
    console.log(global.SearchResult);
    ...
    const columns = [
      {
        ...
        sorter: (a: any, b: any) =>
          (a.title + '-' + a.chapter).localeCompare(b.title + '-' + b.chapter),
      ...
      },
      {
        ...
        // filterSearch: true,
        sorter: (a: any, b: any) =>
          (a.category_id < 10 ? a.category_id * 10 : a.category_id) -
          (b.category_id < 10 ? b.category_id * 10 : b.category_id),
        onFilter: (value: string, record: any) =>
