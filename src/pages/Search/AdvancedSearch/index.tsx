	  ...
	  for (var i=0;i<books.length;i++) {
		  ...
		  for (var j=0;j<books[i].children.length;j++) {
			  ...
			  var page_node = {
				  ...
				  value: '@@@'+book_node.title+'-'+page.name[global.langid]+'-'+j,
				  key: '@@@'+book_node.title+'-'+page.name[global.langid]+'-'+j,
			  };
...
function AdvancedSearch() {
  ...
  const handleFormSubmit = (value: string) => {
	  ...
	  if (advanced) {
		...
		constraint = constraint.getElementsByClassName('ant-select-selection-item')[0].textContent;
  ...
  return (
    <form style={{ marginLeft: '32px' }}>
      ...
      <Row style={{ margin: '6px' }} align="middle">
        <Checkbox id='search_advanced' onChange={handleChange}>使用高级检索式</Checkbox>
      ...
      <Row style={{ margin: '6px' }}>
        检索模式：
        <Checkbox onChange={(value) => {setFuzzy(value.target.checked);}}>模糊查询</Checkbox>
        <Checkbox onChange={(value) => {setSynExp(value.target.checked);}}>同义词扩展</Checkbox>
      </Row>
      <Row style={{ margin: '6px' }}>
        书目范围：
