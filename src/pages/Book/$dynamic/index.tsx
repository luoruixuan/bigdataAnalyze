    ...
    for (var i = 0; i < this.state.data.children.length; i++) {
      ...
      for (var j = 0; j < page.children.length; j++) {
        ...
        for (var k = 0; k < para.length; k++) {
          ...
          if (reused) {
            var sty = {
              backgroundColor: this.state.reuseSwitch ? '#F5A9A9' : '#FFFFFF',
            };
          } else {
          ...
          tmp_para.push(
            <>
              <span style={sty} onClick={jump(href)}>
                {txt}
              </span>
            </>,
          );
            ...
            <Row justify="end">
              <Col>
                ...
                <Switch
                  ...
                  onChange={(value) => {
                    this.setState({ reuseSwitch: value });
                  }}
                />
              </Col>
            </Row>
          </div>
          <div style={{ height: 400, overflow: 'scroll' }}>{tmp}</div>
        </TabPane>,
      );
    }
    return tabpanes;
        ...
        <Col span={18} gutter={16}>
          ...
          >
            ...
            <Card.Grid hoverable={false} style={gridStyle}>
              作者：{this.state.data.author[global.langid]}
            </Card.Grid>
            <Card.Grid hoverable={false} style={gridStyle}>
              编者：{this.state.data.editor[global.langid]}
            </Card.Grid>
            <Card.Grid hoverable={false} style={gridStyle}>
              创作时代：{ctime_str}
            </Card.Grid>
            ...
            <Card.Grid hoverable={false} style={gridStyle}>
              类别：{this.state.data.catalogue[global.langid]}
            </Card.Grid>
          </Card>
          <Card
            style={{ marginTop: 24 }}
